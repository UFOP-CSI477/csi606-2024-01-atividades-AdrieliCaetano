...
import { getByIdLocal } from "@/app/repository/locais/LocalRepository";
import ICidade from "@/app/types/ICidade";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { FormEvent, useEffect, useState } from "react"

interface IUpdateLocalParams {
    params : {
        id: string
    }
}

export default function UpdateLocal( { params }: IUpdateLocalParams ) {
    ...
    async function handleSubmit(event : FormEvent) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.alert(`Local de coleta alterado com sucesso! Id: ${id}`)
            ...
            window.alert('Erro na alteração do local de coleta!')
            console.error(error)
        }
    }
    ...
    return (
        <main>
            <h1 className="text-center font-bold py-4">Atualização de local de coleta:  {nome} </h1>
